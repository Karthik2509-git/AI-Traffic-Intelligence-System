...
void CityController::updateTracks(const std::vector<::traffic::Track>& tracks) {
    ...
    // 1. Calculate REAL density based on detection count
    // Normalized for 1km stretch. Current camera 4K-Alpha covers ~0.5km.
    float current_density = static_cast<float>(tracks.size()) * 2.0f;
    
    // 2. Update the Road Graph for Node 0 (Live Mobile Sensor)
    graph->updateDensity(0, current_density);

    // 3. Compute Global City Pressure based on real-time flow
    global_pressure = current_density; 

    // 4. Trigger Autonomous Signal Decision Pass
    control::RLSignalController::Observation obs;
    obs.lane_densities.push_back(current_density);
    obs.queue_lengths.push_back(static_cast<float>(tracks.size()));
    obs.neighbor_pressures.push_back(0.1f);
    obs.time_of_day = 12.0f; 

    auto action = signal_controller->computePolicy(obs);
    
    if (action.duration_extension > 0) {
        ::traffic::Logger::info("Intelligent Signal Update: Phase " + 
                  std::to_string(action.phase_id) + " Extended by " + 
                  std::to_string(action.duration_extension) + "s");
    }
}
...
void CityController::handleAlerts(const std::vector<analytics::AnomalyDetector::Alert>& alerts) {
    for (const auto& alert : alerts) {
        if (alert.type == analytics::AnomalyDetector::AnomalyType::ACCIDENT_DETECTION) {
            traffic::Logger::warn("EMERGENCY OVERRIDE: Accident at Intersection 0. Green-Wave Active.");
            signal_controller->updateReward(0.0f, -100.0f); 
        }
    }
}
