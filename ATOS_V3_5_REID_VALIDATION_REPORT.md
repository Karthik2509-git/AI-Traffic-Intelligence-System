...
| **4** | **Model Loaded** | `MODEL_FILE_PENDING` | `models/reid_vehiclenet.onnx` weights file is currently un-populated on host disk. System safely reports `"model_loaded": false`. |
| **5** | **Dataset Prepared** | `DATASET_PENDING` | VeRi-776 / CityFlow dataset images not downloaded on host (`datasets/reid/veri776/`). Licensing and manual setup instructions documented in `datasets/reid/README.md`. |
